...
#ifdef OCTET_BULLET
	void add_hinge(btHingeConstraint *constraint)
	{
		world->addConstraint(constraint);
	}
#endif
#ifdef OCTET_BULLET
	void add_spring(btGeneric6DofSpringConstraint *constraint)
	{
		world->addConstraint(constraint);
	}
#endif
#ifdef OCTET_BULLET
	void add_point2point(btPoint2PointConstraint *constraint)
	{
		world->addConstraint(constraint);
	}
#endif
	/// helper to add a mesh to a scene and also to create the corresponding physics object
	scene_node *add_shape_node(mat4t_in mat, mesh *msh, material *mtl, bool is_dynamic = false, float mass = 1, collison_shape_t *shape = NULL) {
